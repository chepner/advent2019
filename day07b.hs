...
runWithFeedback :: Program -> [Int] -> ([Int], Int)
runWithFeedback p as@[a1,a2,a3,a4,a5] = let run :: [Int] -> ([Int], [Int])
                                            run input = execState (eval p 0) (input, [])
                                            (_, runAout) = run (a1: 0 : runEout)
                                            (_, runBout) = run (a2: runAout)
                                            (_, runCout) = run (a3: runBout)
                                            (_, runDout) = run (a4: runCout)
                                            (_, runEout) = run (a5: runDout)
                                        in (as, last runEout)
...
main = do
  ...
  print $ maximumBy (comparing (\(_,o) -> o)) (map (runWithFeedback program) inputs) -- 14260332
